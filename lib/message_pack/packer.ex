defmodule MessagePack.Packer do
  ...
  defp do_pack(%{__struct__: _} = term, %{ext_packer: packer}) when is_function(packer),
    do: pack_ext_wrap(term, packer)

  defp do_pack(map, options) when is_map(map), do: pack_map(Enum.into(map, []), options)

  defp do_pack(term, %{ext_packer: packer}) when is_function(packer),
    do: pack_ext_wrap(term, packer)
  ...
  defp pack_int(i) when i >= -0x8000000000000000,
    do: <<0xD3::8, i::64-big-signed-integer-unit(1)>>
  ...
  defp pack_uint(i) when i < 0x10000000000000000,
    do: <<0xCF::8, i::64-big-unsigned-integer-unit(1)>>
      ...
      {:ok, {type, data}} when type < 0x100 and is_binary(data) ->
        maybe_bin =
          case byte_size(data) do
            1 ->
              <<0xD4, type::8, data::binary>>

            2 ->
              <<0xD5, type::8, data::binary>>

            4 ->
              <<0xD6, type::8, data::binary>>

            8 ->
              <<0xD7, type::8, data::binary>>

            16 ->
              <<0xD8, type::8, data::binary>>
